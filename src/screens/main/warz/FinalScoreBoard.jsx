...
const FinalScoreBoard = ({route}) => {
  ...
  /**
   * 🧠 Rank participants by votes (DESC)
   */
  const {topThree, rest} = useMemo(() => {
    if (!participants.length) return {topThree: [], rest: []};

    // 🔹 Sort participants by vote count descending
    const sortedParticipants = participants
      .map(p => ({
        ...p,
        voteCount: Array.isArray(p.vote) ? p.vote.length : 0, // safely handle votes
      }))
      .sort((a, b) => b.voteCount - a.voteCount); // highest votes first

    return {
      topThree: sortedParticipants.slice(0, 3),
      rest: sortedParticipants.slice(3),
    };
  }, [participants]);

  // 🔹 Debugging: check sorted top 3
  console.log(
    'Top 3 participants:',
    topThree.map(p => ({name: p.participant.name, votes: p.voteCount})),
  );
        ...
        <View
          ...
          <FlatList
            ...
            keyExtractor={(item, index) =>
              item?.participant?._id || index.toString()
            }
            contentContainerStyle={{
              borderWidth: 1,
              borderColor: AppColors.RED,
